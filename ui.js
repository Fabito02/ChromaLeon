...
class ChromaLeonUI {
    constructor(window, page, settings) {
        ...
        const wallpaperGroup = new Adw.PreferencesGroup({
            ...
            description: "The system automatically updates the accent color based on your wallpaper.",
        });
        this._page.add(wallpaperGroup);

        const miscellaneousGroup = new Adw.PreferencesGroup({ title: "Miscellaneous" });
        ...
        this._settings.bind("create-shortcut", shortcutSwitch, "active", Gio.SettingsBindFlags.DEFAULT);
        ...
        this._applyTheme = () => {
            ...
            flatpakSwitch.get_style_context().add_provider(cssProvider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION);
            shortcutSwitch.get_style_context().add_provider(cssProvider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION);
            colorRow.set_subtitle(hex);
        };

        this._applyTheme();

        this._settingsId = this._settings.connect("changed::accent-color", () => this._applyTheme());
        ...
        const checkFlatpakPermissions = () => {
            try {
                const proc = Gio.Subprocess.new(["flatpak", "override", "--user", "--show"], Gio.SubprocessFlags.STDOUT_PIPE);
                const [success, stdout] = proc.communicate_utf8(null, null);
                return (stdout || "").split("filesystems=")[1]?.split(";")?.includes("xdg-config/gtk-3.0") || false;
        ...
        flatpakSwitch.connect("notify::active", () => {
            ...
            const commands = isActive 
                ? [["flatpak", "override", "--user", "--filesystem=xdg-config/gtk-3.0"], ["flatpak", "override", "--user", "--filesystem=xdg-config/gtk-4.0"]]
                : [["flatpak", "override", "--user", "--nofilesystem=xdg-config/gtk-3.0"], ["flatpak", "override", "--user", "--nofilesystem=xdg-config/gtk-4.0"]];
            commands.forEach(cmd => { try { Gio.Subprocess.new(cmd, Gio.SubprocessFlags.NONE); } catch(e){} });
        });

        const previewRow = new Adw.PreferencesRow({ activatable: false });
        this._previewContainer = new Gtk.Box({
            margin_top: 16, margin_bottom: 16, margin_start: 16, margin_end: 16,
            hexpand: true, halign: Gtk.Align.FILL, valign: Gtk.Align.START,
        });
        previewRow.set_child(this._previewContainer);
        wallpaperGroup.add(previewRow);

        this._colorsRow = new Adw.ActionRow({ valign: Gtk.Align.CENTER, halign: Gtk.Align.CENTER });
        this._colorBox = new Gtk.Box({
            orientation: Gtk.Orientation.HORIZONTAL, spacing: 8,
            valign: Gtk.Align.CENTER, halign: Gtk.Align.CENTER, margin_top: 10, margin_bottom: 10,
        });
        ...
        this._bgChangedId = this._bgSettings.connect("changed::picture-uri-dark", () => this._updateWallpaperUI());
    ...
    _updateWallpaperUI() {
        while (this._previewContainer.get_first_child()) this._previewContainer.remove(this._previewContainer.get_first_child());
        if (this._colorBox) { while (this._colorBox.get_first_child()) this._colorBox.remove(this._colorBox.get_first_child()); }

        let uri = this._bgSettings.get_string("picture-uri-dark");
        if (uri.startsWith("file://")) {
            let file = Gio.File.new_for_uri(uri);
            let preview = new Gtk.Picture({ file: file, can_shrink: true, content_fit: Gtk.ContentFit.COVER });
            preview.add_css_class("card");

            let aspectFrame = new Gtk.AspectFrame({
                ratio: 16.0 / 9.0, child: preview, obey_child: false,
                hexpand: true, halign: Gtk.Align.FILL, valign: Gtk.Align.START, height_request: 240,
            });
        ...
        if (colors.length > 0) {
            colors.forEach((hexColor) => {
                let btn = new Gtk.Button({ valign: Gtk.Align.CENTER, halign: Gtk.Align.CENTER });
                let cssProvider = new Gtk.CssProvider();
                let cssString = `button { background-color: ${hexColor}; min-width: 30px; min-height: 30px; border-radius: 50%; padding: 0; }`;
                
                if (cssProvider.load_from_string) cssProvider.load_from_string(cssString);
                ...
                btn.connect("clicked", () => this._settings.set_string("accent-color", hexColor));
    ...
    _getWallpaperColors() {
        ...
        let file = Gio.File.new_for_uri(uri);
        try {
            let pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(file.get_path(), 64, 64, true);
            let pixels = pixbuf.get_pixels(), rowstride = pixbuf.get_rowstride(), channels = pixbuf.get_n_channels();
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    ...
                    if (!colorMap.has(hex)) colorMap.set(hex, { count: 0, h: h, s: s, l: l });
            ...
            for (let [hex, data] of colorMap.entries()) {
                colorsList.push({ hex: hex, count: data.count, h: data.h, s: data.s, l: data.l });
            }

            let vibrantRanking = [...colorsList].sort((a, b) => {
                let scoreA = a.s * a.s * (1 - Math.abs(a.l - 50) / 50) * Math.log(a.count + 1);
                let scoreB = b.s * b.s * (1 - Math.abs(b.l - 50) / 50) * Math.log(b.count + 1);
                return scoreB - scoreA;
            });

            let finalColors = [], usedHues = [];
            for (let color of vibrantRanking) {
                ...
                for (let usedHue of usedHues) {
                    ...
                    if (hueDiff < 25) { isTooSimilar = true; break; }
                }
                if (!isTooSimilar) { finalColors.push(color.hex); usedHues.push(color.h); }
